...
if (!isVideo) {
  // 自检: Safari 访问 https://finder.video.qq.com/sphping 应弹通知并打心跳
  if (/sphping/i.test(url)) {
    $notify("视频号脚本已生效", "MITM + 重写命中", url.slice(0, 60), { openUrl: CFG.listView });
    if (CFG.enableCollector) {
      $task.fetch({ method: "POST", url: CFG.collector.replace("/report", "/ping"),
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
        body: "url=" + encodeURIComponent(url) }).then(() => $done({}), () => $done({}));
    } else { $done({}); }
  } else { reply(); }
}
else if (!seen()) { reply(); }
